#!/usr/bin/env python3
# finalize.py - writes the COMPLETE FlowSentry app (backend+frontend) on port 9000,
# shows which version was on disk before, and verifies data + imports.
import os
import sys
...
FILES["dashboard/app.py"] = r'''"""FlowSentry backend. Run: python -m dashboard.app -> http://localhost:9000"""
import json
import os
...
FILES["app.py"] = r'''"""Root launcher: python app.py -> http://localhost:9000"""
...
FILES["dashboard/static/style.css"] = r''':root{
...
FILES["dashboard/static/app.js"] = r'''/* FlowSentry v5 - working tabs, scenario pills, defensive rendering. */
...
def main():
    # evidence: what was on disk before this run
    old = os.path.join(ROOT, "dashboard", "templates", "index.html")
    if os.path.exists(old):
        with open(old, encoding="utf-8", errors="replace") as f:
            head = f.read(200).replace("\n", " ")
        print("BEFORE this script, index.html started with:")
        print("   ", head[:150])
        print("(If that mentions AquaGuard dark style - the other session had overwritten it.)")
    print()
    ...
    has_data = os.path.exists(os.path.join(ROOT, "dashboard", "data", "results.json"))
    print()
    if has_data:
        print("results.json: FOUND - dashboard will load directly.")
    else:
        print("results.json: MISSING - after this, run:  python run_pipeline.py")
    print()
    print("DONE. Start with:  python -m dashboard.app   ->   http://localhost:9000")
    print("IMPORTANT: do not run any scripts from the other chat session anymore -")
    print("they overwrite these files with older versions.")
